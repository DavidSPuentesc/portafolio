...
it('links every headline metric to the case that gives it context', async () => {
  ...
  expect(html.match(/<a href="\/es\/projects\/(securapp|smartsense)\/">/g)).toHaveLength(4);
  ...
  expect(html).toContain('<strong>688 m</strong>');
  expect(html).toMatch(/<small>[^<]*8 semanas[^<]*<\/small>/);
  expect(html).toMatch(/<small>Cota inferior[^<]*<\/small>/);
  ...
  expect(english).toMatch(/Lower bound/);
});

...
it('links every headline metric to the case that gives it context', async () => {
  ...
  expect(html.match(/<a href="\/es\/projects\/securapp\/">/g)).toHaveLength(4);
  ...
  expect(html).toContain('<strong>5</strong><span>frentes operativos modernizados</span>');
  expect(html).not.toContain('<strong>688 m</strong>');
  expect(html).toMatch(/<small>[^<]*8 semanas[^<]*<\/small>/);
  expect(html).toMatch(/<small>Contratistas, visitas, planes de acción, roles y PTW[^<]*<\/small>/);
  ...
  expect(english).toContain('modernized operational areas');
});
